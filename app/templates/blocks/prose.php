 ...
 */ ?>
<section class="prose-sec" <?= raw_html(!empty($b['id']) ? 'id="' . e($b['id']) . '"' : '') ?>>
  <div class="wrap split split--wide">
    ...
    <div class="prose">
      <?php foreach ($b['body_html'] as $part): ?>
        <?php if (is_string($part)): ?>
          <p><?= raw_html($part) ?></p>
        <?php elseif (($part['type'] ?? '') === 'list'): ?>
          <ul class="prose__list">
            <?php foreach ($part['items_html'] as $item): ?><li><?= raw_html($item) ?></li><?php endforeach; ?>
          </ul>
        ...
        <?php elseif (($part['type'] ?? '') === 'defs'): ?>
          <dl class="defs">
            <?php foreach ($part['items'] as $d): ?>
              <div class="defs__row"><dt><?= e($d['term']) ?></dt><dd><?= raw_html($d['def_html']) ?></dd></div>
            <?php endforeach; ?>
          </dl>
        <?php endif; ?>
      <?php endforeach; ?>
