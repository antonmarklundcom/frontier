...
<section class="form-sec" id="<?= e($b['id'] ?? 'enquiry') ?>">
  <div class="wrap split split--wide">
    <div class="split__aside">
      ...
      <?php foreach ((array) ($b['intro'] ?? []) as $p): ?><p><?= $p ?></p><?php endforeach; ?>
    </div>
    ...
    <div class="formwrap">
      ...
      <form class="form" method="post" action="<?= e(href(page_url('book'))) ?>#enquiry" novalidate>
        ...
        <div class="form__row">
          ...
          <input class="form__input<?= $err('name') ? ' is-error' : '' ?>" type="text" id="f-name" name="name"
                 autocomplete="name" required value="<?= $val('name') ?>" <?= $enabled ? '' : 'disabled' ?>
                 <?= $err('name') ? 'aria-describedby="e-name" aria-invalid="true"' : '' ?>>
        ...
        <div class="form__row">
          ...
          <input class="form__input<?= $err('email') ? ' is-error' : '' ?>" type="email" id="f-email" name="email"
                 autocomplete="email" required value="<?= $val('email') ?>" <?= $enabled ? '' : 'disabled' ?>
                 <?= $err('email') ? 'aria-describedby="e-email" aria-invalid="true"' : '' ?>>
        ...
        <div class="form__grid">
          <div class="form__row">
            ...
            <input class="form__input" type="text" id="f-country" name="country" autocomplete="country-name"
                   value="<?= $val('country') ?>" <?= $enabled ? '' : 'disabled' ?>>
          ...
          <div class="form__row">
            ...
            <select class="form__input" id="f-stage" name="stage" <?= $enabled ? '' : 'disabled' ?>>
              <?php
              $stages = [
                  '' => 'Select one',
              ...
              foreach ($stages as $k => $label): ?>
                <option value="<?= e($k) ?>"<?= ($old['stage'] ?? '') === $k && $k !== '' ? ' selected' : '' ?>><?= e($label) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        ...
        <div class="form__row">
          ...
          <textarea class="form__input form__textarea<?= $err('message') ? ' is-error' : '' ?>" id="f-message"
                    name="message" rows="7" required <?= $enabled ? '' : 'disabled' ?>
                    <?= $err('message') ? 'aria-describedby="e-message" aria-invalid="true"' : '' ?>><?= $val('message') ?></textarea>
        ...
        <div class="form__foot">
          <button class="btn btn--primary" type="submit" <?= $enabled ? '' : 'disabled' ?>
