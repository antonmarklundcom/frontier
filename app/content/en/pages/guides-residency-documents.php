...
'faqs' => [
  ['q' => 'Does every document need an apostille?',
   'a_html' => ['{{ 60-90 words: answer directly, then name the exception class (documents issued inside Paraguay, and countries not party to the Hague Convention, which use consular legalisation instead). }}']],
  ['q' => 'How long does an apostille stay valid?',
   'a_html' => ['{{ 60-90 words: distinguish the apostille itself from the validity window Migraciones applies to the underlying record. This distinction is the single most common misunderstanding on this page — make it unmissable. }}']],
  ['q' => 'Can I get documents translated in my own country?',
   'a_html' => ['{{ 60-90 words: explain the sworn-translator requirement and who is accepted. State plainly whether a translation done abroad is usable. }}']],
  ['q' => 'What happens if a document is rejected?',
   'a_html' => ['{{ 60-90 words: what re-submission actually involves, what has to be re-obtained versus re-authenticated, and why ordering records in the right sequence prevents most of it. No time estimate. }}']],
],
...
['type' => 'quick-answer',
 ...
 'caveat_html' => '{{ 30-50 words: the conditions under which this general answer does not hold — nationality, document age, applicants with prior immigration history, and any record issued by a state not party to the Hague Convention. }}',
],
...
['type' => 'checklist',
 ...
 'groups' => [
   ['title' => 'Obtained in your own country, before you travel',
    ...
    'items' => [
      ['item_html' => '{{ record 1 — name it as Migraciones names it, with the English equivalent in brackets }}',
       'note_html' => '{{ 25-45 words: where it is issued, what it must show, the most common reason this one is rejected }}',
       'who' => 'You', 'validity' => '{{ validity window, or "no stated limit" }}'],
      ['item_html' => '{{ record 2 }}', 'note_html' => '{{ 25-45 words: as above }}', 'who' => 'You', 'validity' => '{{ validity window }}'],
      ['item_html' => '{{ record 3 }}', 'note_html' => '{{ 25-45 words: as above }}', 'who' => 'You', 'validity' => '{{ validity window }}'],
      ['item_html' => '{{ record 4 — include only if genuinely required; delete this row rather than pad the list }}',
       'note_html' => '{{ 25-45 words: as above }}', 'who' => 'You', 'validity' => '{{ validity window }}'],
    ]],
   ['title' => 'Authentication of those records',
    ...
    'items' => [
      ['item_html' => '{{ authentication step for Hague Convention countries }}',
       'note_html' => '{{ 25-45 words: which national authority does this, and what the reader physically ends up holding }}', 'who' => 'You'],
      ['item_html' => '{{ authentication route for non-Hague countries }}',
       'note_html' => '{{ 25-45 words: the consular chain, described honestly as slower }}', 'who' => 'You'],
      ['item_html' => '{{ sworn translation requirement }}',
       'note_html' => '{{ 25-45 words: who is permitted to produce it and whether it happens before or after arrival }}', 'who' => 'Us'],
    ]],
   ['title' => 'Obtained inside Paraguay',
    'note'  => '{{ 25-40 words: why these cannot be started from abroad. }}',
    'items' => [
      ['item_html' => '{{ Paraguayan record 1 }}', 'note_html' => '{{ 25-45 words }}', 'who' => 'Us'],
      ['item_html' => '{{ Paraguayan record 2 }}', 'note_html' => '{{ 25-45 words }}', 'who' => 'Us'],
      ['item_html' => '{{ Paraguayan record 3 }}', 'note_html' => '{{ 25-45 words }}', 'who' => 'Us'],
    ]],
 ],
 'footnote_html' => '{{ 30-50 words: state that this checklist is dated, that requirements change, and how to report an error. Do not promise completeness for every nationality. }}',
],

['type' => 'steps',
 'eyebrow' => 'Sequence',
 'heading' => 'The order to obtain them in',
 'intro_html' => ['{{ 40-60 words: the argument for sequence — the goal is that no record expires while another is still being produced. }}'],
 'items' => [
   ['title' => '{{ stage 1 title — the longest-lead record }}',
    'body_html' => ['{{ 50-80 words: what to request, from whom, and what to check on it the day it arrives }}'],
    'who' => 'You',
    'blocker_html' => '{{ 20-35 words: the specific thing that most often goes wrong at this stage }}'],
   ['title' => '{{ stage 2 title }}', 'body_html' => ['{{ 50-80 words }}'], 'who' => 'You',
    'blocker_html' => '{{ 20-35 words }}'],
   ['title' => '{{ stage 3 title — authentication }}', 'body_html' => ['{{ 50-80 words }}'], 'who' => 'You'],
   ['title' => '{{ stage 4 title — translation and Paraguayan-side preparation }}',
    'body_html' => ['{{ 50-80 words }}'], 'who' => 'Us'],
   ['title' => '{{ stage 5 title — assembly and filing }}', 'body_html' => ['{{ 50-80 words }}'], 'who' => 'Us'],
 ],
 'footnote_html' => '{{ 25-40 words: no total duration is given here, and say why — the stages we do not control are not ours to estimate. }}',
],
...
['type' => 'prose',
 ...
 'body_html' => [
   ...
   ['type' => 'defs', 'items' => [
     ['term' => 'Married applicants and dependants', 'def_html' => '{{ 40-70 words: additional records, and the one that is most often forgotten }}'],
     ['term' => 'Applicants with a name that has changed', 'def_html' => '{{ 40-70 words: how the chain of records has to line up }}'],
     ['term' => 'Records from a country not party to the Hague Convention', 'def_html' => '{{ 40-70 words: the consular route }}'],
     ['term' => 'Applicants with a criminal record or a prior refusal', 'def_html' => '{{ 40-70 words: state plainly that concealment is the real risk, and point at the integrity page }}'],
   ]],
 ],
],
...
['type' => 'sources',
 'intro_html' => ['{{ 25-40 words: one sentence on which of these was primary for this page. }}'],
 'items' => [
   ['name' => 'Dirección Nacional de Migraciones', 'authority' => 'Paraguayan immigration authority',
    ...
    'note_html' => '{{ 15-30 words: what specifically was taken from it }}'],
   ...
   ['name' => '{{ any further primary source relied on — delete this row if there is none }}',
    'authority' => '{{ issuing body }}', 'url' => '{{ exact URL used }}', 'note_html' => '{{ 15-30 words }}'],
 ],
],
...
['type' => 'next-step',
 ...
 'body_html' => ['{{ 40-60 words: what a consultation adds over this page — a checklist for the reader\'s own nationality and situation, and the sequence applied to their dates. }}'],
 'cta' => ['page' => 'book', 'label' => 'Book a consultation'],
 'secondary' => ['page' => 'services.residency', 'label' => 'What our residency support covers'],
 'diy_html' => '{{ 30-50 words: the honest note — the kind of applicant who does not need us, stated without hedging. This appears on several guides and must never read as false modesty. }}',
],
