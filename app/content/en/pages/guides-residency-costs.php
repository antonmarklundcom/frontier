...
'faqs' => [
  ['q' => 'Why do you not publish one all-in price?',
   'a_html' => ['{{ 60-90 words: because an all-in number hides which parts are ours, which are the government\'s and which vary with the reader\'s own records. Say what we publish instead. }}']],
  ['q' => 'Which costs do I pay directly to someone else?',
   'a_html' => ['{{ 60-90 words: name the categories paid to third parties and confirm we do not mark them up. }}']],
  ['q' => 'What makes a case more expensive than the common one?',
   'a_html' => ['{{ 60-90 words: the specific complications — records from multiple countries, name changes, dependants, non-Hague authentication. }}']],
  ['q' => 'What are the ongoing costs after approval?',
   'a_html' => ['{{ 60-90 words: renewals, tax filing obligations once registered, and anything else that recurs. }}']],
],
...
['type' => 'quick-answer',
 ...
 'caveat_html' => '{{ 30-50 words: fees change, published amounts go stale, and the date at the foot of the page is the one that matters. }}',
],
...
['type' => 'comparison',
 ...
 'rows' => [
   ['label' => 'Government fees',              'cells_html' => ['{{ recipient }}', '{{ fixed/variable }}', '{{ 12-25 words }}']],
   ['label' => 'Apostille and legalisation',   'cells_html' => ['{{ recipient }}', '{{ fixed/variable }}', '{{ 12-25 words }}']],
   ['label' => 'Sworn translation',            'cells_html' => ['{{ recipient }}', '{{ fixed/variable }}', '{{ 12-25 words }}']],
   ['label' => 'Professional fees',            'cells_html' => ['{{ recipient }}', '{{ fixed/variable }}', '{{ 12-25 words }}']],
   ['label' => 'Travel and time in country',   'cells_html' => ['{{ recipient }}', '{{ fixed/variable }}', '{{ 12-25 words }}']],
   ['label' => 'Ongoing, after approval',      'cells_html' => ['{{ recipient }}', '{{ fixed/variable }}', '{{ 12-25 words }}']],
 ],
 'footnote_html' => '{{ 30-50 words: state which amounts are published by an authority and when they were last checked. Our own fees are on the packages page, not here, and say so. }}',
],
...
['type' => 'sources',
 'intro_html' => ['{{ 25-40 words: where the published fee amounts come from and the date they were checked. }}'],
 'items' => [
   ['name' => 'Dirección Nacional de Migraciones', 'authority' => 'Published fee schedule',
    'url' => '{{ exact URL used }}', 'note_html' => '{{ 15-30 words: which fees, and the date checked }}'],
...
['type' => 'next-step',
 ...
 'body_html' => ['{{ 40-60 words: what the consultation produces — a written breakdown by category with the uncertain parts marked as uncertain. }}'],
 'cta' => ['page' => 'book', 'label' => 'Book a consultation'],
 'secondary' => ['page' => 'packages', 'label' => 'What our packages include'],
 'diy_html' => '{{ 30-50 words: the reader who can do this alone and should. }}',
],
